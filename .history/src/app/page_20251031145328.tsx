...
export default function Home() {
  ...
  return (
    <>
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-4 px-4">
          <h2 className="text-lg md:text-xl font-bold text-gray-800">🛍️ All Products</h2>
          <Link href="/search" className="flex items-center text-sm text-brand hover:underline">
            View All
            <svg className="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
        ...
        <div className="px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {[...products.filter(p => p.quantity > 0), ...FASHION_PRODUCTS.filter(p => p.quantity > 0)].slice(0, 12).map((product, index) => (
              <div key={`all-${product.id}-${index}`}>
                <SafeProductCard p={product} />
              </div>
            ))}
          </div>
